...
import { cancel_retryable } from "./cancel-retryable"
...
const SELECTOR = function_selector("cancel", [
  bytes32_codec(),
])
  ...
  return {
    ...
    l1: {
      ...
      reader: async (_call: Call): Promise<Response> => {
        throw new Error(
          "l1 reader should not be invoked from cancel_retryable",
        )
      },
    },
    l2: {
...
describe("cancel_retryable", () => {
  it("signs ArbRetryableTx.cancel on L2 and broadcasts via eth_sendRawTransaction", async () => {
    ...
    const hash = await cancel_retryable({
      ticket_id: TICKET_ID,
    })(resolved)
  ...
  it("throws when signer is undefined", async () => {
    ...
    await expect(
      cancel_retryable({ ticket_id: TICKET_ID })(resolved),
