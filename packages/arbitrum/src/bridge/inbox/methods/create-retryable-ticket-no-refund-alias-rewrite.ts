import {
  address,
  bytes,
  encode_function_call,
  uint256,
} from "@ethernauta/abi"
import type { Bytes } from "@ethernauta/core"
import {
  AddressSchema,
  BytesSchema,
  Uint256Schema,
  UintSchema,
} from "@ethernauta/core"
import { eth_signTransaction } from "@ethernauta/eth"
import type {
  ResolvedSigner,
  Signable,
} from "@ethernauta/transport"
import { bytes_to_hex } from "@ethernauta/utils"
import type { InferOutput } from "valibot"
import { object, parse, tuple, union } from "valibot"

const PARAM_CODECS = [
  address(),
  uint256(),
  uint256(),
  address(),
  address(),
  uint256(),
  uint256(),
  bytes(),
] as const

export const CREATE_RETRYABLE_TICKET_NO_REFUND_ALIAS_REWRITE_SIGNATURE =
  {
    signature:
      "createRetryableTicketNoRefundAliasRewrite(address,uint256,uint256,address,address,uint256,uint256,bytes)",
    names: [
      "to",
      "l2CallValue",
      "maxSubmissionCost",
      "excessFeeRefundAddress",
      "callValueRefundAddress",
      "gasLimit",
      "maxFeePerGas",
      "data",
    ],
  }

const ParametersSchema = union([
  tuple([
    AddressSchema,
    Uint256Schema,
    Uint256Schema,
    AddressSchema,
    AddressSchema,
    Uint256Schema,
    Uint256Schema,
    BytesSchema,
  ]),
  object({
    to: AddressSchema,
    l2CallValue: Uint256Schema,
    maxSubmissionCost: Uint256Schema,
    excessFeeRefundAddress: AddressSchema,
    callValueRefundAddress: AddressSchema,
    gasLimit: Uint256Schema,
    maxFeePerGas: Uint256Schema,
    data: BytesSchema,
  }),
])
type Parameters = InferOutput<typeof ParametersSchema>

export function createRetryableTicketNoRefundAliasRewrite(
  _parameters: Parameters,
): Signable<Bytes> {
  return async ([
    signer,
    context,
  ]: ResolvedSigner): Promise<Bytes> => {
    if (!context.to)
      throw new Error(
        "contract Signable requires a 'to' on the signer resolver",
      )
    const parameters = parse(ParametersSchema, _parameters)
    const values = Array.isArray(parameters)
      ? ([
          parameters[0],
          parameters[1],
          parameters[2],
          parameters[3],
          parameters[4],
          parameters[5],
          parameters[6],
          parameters[7],
        ] as const)
      : ([
          parameters.to,
          parameters.l2CallValue,
          parameters.maxSubmissionCost,
          parameters.excessFeeRefundAddress,
          parameters.callValueRefundAddress,
          parameters.gasLimit,
          parameters.maxFeePerGas,
          parameters.data,
        ] as const)
    const calldata = encode_function_call({
      name: "createRetryableTicketNoRefundAliasRewrite",
      args: PARAM_CODECS,
      values,
    })
    // TODO(wallet): wallet fills nonce, gas, gasPrice / maxFeePerGas /
    //               maxPriorityFeePerGas by querying the network
    //               (eth_getTransactionCount, eth_estimateGas, eth_feeHistory).
    //               Generator MUST leave these fields unset.
    return eth_signTransaction([
      {
        to: context.to,
        value: context.value ?? parse(UintSchema, "0x0"),
        input: parse(BytesSchema, bytes_to_hex(calldata)),
        _ethernauta: {
          function:
            CREATE_RETRYABLE_TICKET_NO_REFUND_ALIAS_REWRITE_SIGNATURE,
        },
      },
    ])([signer, context])
  }
}
