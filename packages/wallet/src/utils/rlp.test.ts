...
describe("RLP encoding", () => {
  ...
  it("should encode multi-byte numbers correctly", () => {
    const result = encode_rlp(256) // 0x0100
    expect(result).toEqual(new Uint8Array([0x82, 0x01, 0x00])) // 0x80 + 2 bytes
  })
  ...
  it("should encode nested arrays correctly", () => {
    ...
    expect(result[1]).toBe(0x87) // Inner list prefix
  ...
  it("should encode mixed data structure correctly", () => {
    ...
    expect(result[0]).toBe(0xd2) // List prefix for 18 bytes total
    expect(result.length).toBe(19) // 1 prefix + 18 content bytes
