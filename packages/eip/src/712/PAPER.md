...
| Authors | Remco Bloemen ([@Recmo](https://github.com/Recmo)), Leonid Logvinov ([@LogvinovLeon](https://github.com/LogvinovLeon)), Jacob Evans ([@dekz](https://github.com/dekz)) |
| --- | --- |
| Created | 2017-09-12 |
| Requires | [EIP-155](/EIPS/eip-155), [EIP-191](/EIPS/eip-191) |
...
* Specification
  ...
  * Specification of the eth\_signTypedData JSON RPC
...
![eth\_sign screenshot](/assets/eip-712/eth_sign.png)
...
![eth\_signTypedData screenshot](/assets/eip-712/eth_signTypedData.png)
...
The encoding is compliant with [ERC-191](/EIPS/eip-191). The ‘version byte’ is fixed to `0x01`, the ‘version specific data’ is the 32-byte domain separator `domainSeparator` and the ‘data to sign’ is the 32-byte `hashStruct(message)`.
...
* `uint256 chainId` the [EIP-155](/EIPS/eip-155) chain id. The user-agent *should* refuse signing if it does not match the currently active chain.
...
An example how to use Solidity ecrecover to verify the signature calculated with `eth_signTypedData` can be found in the [Example.js](/assets/eip-712/Example.js). The contract is deployed on the testnet Ropsten and Rinkeby.

#### personal\_signTypedData
...
**Alternative 3**: ABIv2 function signatures modified to be 256-bit. While this captures type info, it does not capture any of the semantics other than the function. This is already causing a practical collision between [ERC-20](/EIPS/eip-20)’s and [ERC-721](/EIPS/eip-721)’s `transfer(address,uint256)`, where in the former the `uint256` refers to an amount and the latter to a unique id. In general ABIv2 favors compatibility where a hashing standard should prefer incompatibility.
...
An example contract can be found in [Example.sol](/assets/eip-712/Example.sol) and an example implementation of signing in JavaScript in [Example.js](/assets/eip-712/Example.js)
...
Remco Bloemen ([@Recmo](https://github.com/Recmo)), Leonid Logvinov ([@LogvinovLeon](https://github.com/LogvinovLeon)), Jacob Evans ([@dekz](https://github.com/dekz)), "EIP-712: Typed structured data hashing and signing," *Ethereum Improvement Proposals*, no. 712, September 2017. Available: https\://eips.ethereum.org/EIPS/eip-712.
...
| command                                                                | description               |
|------------------------------------------------------------------------|---------------------------|
| curl.md https://eips.ethereum.org/EIPS/eip-712 --objective <objective> | focus on a specific topic |
