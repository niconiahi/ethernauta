...
[Standards Track: Core](/core)

# EIP-7702: Set Code for EOAs

### Add a new tx type that permanently sets the code for an EOA

| Authors | Vitalik Buterin ([@vbuterin](https://github.com/vbuterin)), Sam Wilson ([@SamWilsn](https://github.com/SamWilsn)), Ansgar Dietrichs ([@adietrichs](https://github.com/adietrichs)), lightclient ([@lightclient](https://github.com/lightclient)) |
| --- | --- |
| Created | 2024-05-07 |
| Requires | [EIP-2](/EIPS/eip-2), [EIP-161](/EIPS/eip-161), [EIP-1052](/EIPS/eip-1052), [EIP-2718](/EIPS/eip-2718), [EIP-2929](/EIPS/eip-2929), [EIP-2930](/EIPS/eip-2930), [EIP-3541](/EIPS/eip-3541), [EIP-3607](/EIPS/eip-3607), [EIP-4844](/EIPS/eip-4844) |
...
* **Batching**: allowing multiple operations from the same user in one atomic transaction. One common example is an [ERC-20](/EIPS/eip-20) approval followed by spending that approval. This is a common workflow in DEXes that requires two transactions today. Advanced use cases of batching occasionally involve dependencies: the output of the first operation is part of the input to the second operation.
...
A new [EIP-2718](/EIPS/eip-2718) transaction known as the “set code transaction” is introduced, where the `TransactionType` is `SET_CODE_TX_TYPE` and the `TransactionPayload` is the RLP serialization of the following:
...
The fields `chain_id`, `nonce`, `max_priority_fee_per_gas`, `max_fee_per_gas`, `gas_limit`, `destination`, `value`, `data`, and `access_list` of the outer transaction follow the same semantics as [EIP-4844](/EIPS/eip-4844). *Note, this implies a null destination is not valid.*
...
3. Let `authority = ecrecover(msg, y_parity, r, s)`.
   ...
   * Verify `s` is less than or equal to `secp256k1n/2`, as specified in [EIP-2](/EIPS/eip-2).

4. Add `authority` to `accessed_addresses`, as defined in [EIP-2929](/EIPS/eip-2929).
...
Delegation indicators use the banned opcode `0xef`, defined in [EIP-3541](/EIPS/eip-3541), to indicate that the code must be handled differently than regular code. The delegation forces all code executing operations to follow the address pointer to obtain the code to execute. For example, `CALL` loads the code at `address` and executes it in the context of `authority`.
...
The intrinsic cost of the new transaction is inherited from [EIP-2930](/EIPS/eip-2930), specifically `21000 + 16 * non-zero calldata bytes + 4 * zero calldata bytes + 1900 * access list storage key count + 2400 * access list address count`. Additionally, add a cost of `PER_EMPTY_ACCOUNT_COST * authorization list length`.

The transaction sender will pay for all authorization tuples, regardless of validity or duplication.

If a code executing instruction accesses a cold account during the resolution of delegated code, add an additional [EIP-2929](/EIPS/eip-2929) `COLD_ACCOUNT_READ_COST` cost of `2600` gas to the normal cost and add the account to `accessed_addresses`. Otherwise, assess a `WARM_STORAGE_READ_COST` cost of `100`.

#### Transaction origination

Modify the restriction put in place by [EIP-3607](/EIPS/eip-3607) to allow EOAs whose code is a valid delegation indicator, i.e. `0xef0100 || address`, to originate transactions. Accounts with any other code values may not originate transactions.
...
Additionally, initcode tends to be propagated inside the transaction calldata. This means it would need to be included in the authorization tuple and signed over. The minimum initcode is around 15 bytes – it would simply copy the contract code from an external address. The total cost would be `16 * 15 = 240` calldata cost, plus the [EIP-3860](/EIPS/eip-3860) cost of `2 * 15 = 30`, plus the runtime costs of around `150`. So nearly `500` additional gas would be spent preparing the account. Even more likely, 1200+ gas if not copying from an external account.
...
This EIP is designed to be forward-compatible with endgame account abstraction, without over-enshrining any fine-grained details of [ERC-4337](/EIPS/eip-4337) or RIP-7560.
...
Vitalik Buterin ([@vbuterin](https://github.com/vbuterin)), Sam Wilson ([@SamWilsn](https://github.com/SamWilsn)), Ansgar Dietrichs ([@adietrichs](https://github.com/adietrichs)), lightclient ([@lightclient](https://github.com/lightclient)), "EIP-7702: Set Code for EOAs," *Ethereum Improvement Proposals*, no. 7702, May 2024. Available: https\://eips.ethereum.org/EIPS/eip-7702.
...
| command                                                                 | description               |
|-------------------------------------------------------------------------|---------------------------|
| curl.md https://eips.ethereum.org/EIPS/eip-7702 --objective <objective> | focus on a specific topic |
