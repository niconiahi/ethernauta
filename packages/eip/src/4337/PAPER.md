...
📢 Last Call [Standards Track: ERC](/erc)

# ERC-4337: Account Abstraction Using Alt Mempool

### Account abstraction without consensus-layer protocol changes, instead relying on higher-layer infrastructure.

| Authors | Vitalik Buterin ([@vbuterin](https://github.com/vbuterin)), Yoav Weiss ([@yoavw](https://github.com/yoavw)), Dror Tirosh ([@drortirosh](https://github.com/drortirosh)), Shahaf Nacson ([@shahafn](https://github.com/shahafn)), Alex Forshtat ([@forshtat](https://github.com/forshtat)), Kristof Gazso ([@kristofgazso](https://github.com/kristofgazso)), Tjaden Hess ([@tjade273](https://github.com/tjade273)) |
| --- | --- |
| Created | 2021-09-29 |
| Last Call Deadline | 2026-05-26 |
| Requires | [EIP-712](/EIPS/eip-712), [EIP-7702](/EIPS/eip-7702) |



[This EIP is in the process of being peer-reviewed. If you are interested in this EIP, please participate using this discussion link.](https://ethereum-magicians.org/t/erc-4337-account-abstraction-via-entry-point-contract-specification/7160)
...
* **Support other use cases**
  ...
  * Atomic multi-operations (similar goal to [EIP-7702](/EIPS/eip-7702))
  * Pay tx fees with [ERC-20](/EIPS/eip-20) tokens, allow developers to pay fees for their users, and [EIP-7702](/EIPS/eip-7702)-like **sponsored transaction** use cases more generally
...
Note that one [EIP-7702](/EIPS/eip-7702) “authorization tuple” value can be provided alongside the `UserOperation` struct, but “authorization tuples” are not included in the `UserOperation` itself.
...
* **Create the `sender` Smart Contract Account if it does not yet exist**, using the `initcode` provided in the `UserOperation`.
  ...
  * If the `factory` address is “0x7702”, then the sender MUST be an EOA with an [EIP-7702](/EIPS/eip-7702) authorization designation. The `EntryPoint` validates the authorized address matches the one specified in the `UserOperation` signature (see Support for \[EIP-7702] authorizations).
...
### JSON-RPC API for [ERC-4337](/EIPS/eip-4337)
...
### Support for [EIP-712](/EIPS/eip-712) signatures
...
Note that a `UserOperation` may still be executed without such `initCode`. In this case the `EntryPoint` doesn’t hash the current [EIP-7702 delegate](/EIPS/eip-7702), and can be potentially executed against a modified account.
...
* If `initCode` is not empty, parse its first 20 bytes as a factory address or an [EIP-7702](/EIPS/eip-7702) flag.\
...
* The calldata gas cost related to the `UserOperation` as defined in [EIP-2028](/EIPS/eip-2028).
...
* [EIP-7623](/EIPS/eip-7623) calldata floor price increase is estimated as follows:
...
One common approach to this problem is often referred to as “diamond storage” and is fully described in [ERC-7201](/EIPS/eip-7201).

### Transient Storage

Contracts using the [EIP-1153](/EIPS/eip-1153) transient storage MUST take into account that ERC-4337 allows multiple `UserOperations` from different unrelated `sender` addresses to be included in the same underlying transaction. The transient storage MUST be cleaned up manually if contains any sensitive information or is used for access control.
...
2. `UserOperations` that are valid when checked independently but fail when bundled together to be put on-chain. To prevent such rogue `UserOperations`, the bundler is required to follow a set of shared set of rules applied to the validation code, to prevent such denial-of-service attacks.
...
Vitalik Buterin ([@vbuterin](https://github.com/vbuterin)), Yoav Weiss ([@yoavw](https://github.com/yoavw)), Dror Tirosh ([@drortirosh](https://github.com/drortirosh)), Shahaf Nacson ([@shahafn](https://github.com/shahafn)), Alex Forshtat ([@forshtat](https://github.com/forshtat)), Kristof Gazso ([@kristofgazso](https://github.com/kristofgazso)), Tjaden Hess ([@tjade273](https://github.com/tjade273)), "ERC-4337: Account Abstraction Using Alt Mempool \[DRAFT]," *Ethereum Improvement Proposals*, no. 4337, September 2021. Available: https\://eips.ethereum.org/EIPS/eip-4337.
...
| command                                                                 | description               |
|-------------------------------------------------------------------------|---------------------------|
| curl.md https://eips.ethereum.org/EIPS/eip-4337 --objective <objective> | focus on a specific topic |
