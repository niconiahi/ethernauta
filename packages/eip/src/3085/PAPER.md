...
🚧 Stagnant [Standards Track: Interface](/interface)

# EIP-3085: wallet\_addEthereumChain RPC Method

### Adds an RPC method to add EVM-compatible chains

| Authors | Erik Marks ([@rekmarks](https://github.com/rekmarks)), Pedro Gomes ([@pedrouid](https://github.com/pedrouid)), Pandapip1 ([@Pandapip1](https://github.com/Pandapip1)) |
| --- | --- |
| Created | 2020-11-01 |
| Discussion Link | <https://ethereum-magicians.org/t/eip-3085-wallet-addethereumchain/5469> |
| Requires | [EIP-155](/EIPS/eip-155) |
...
The `chainId` is the integer ID of the chain as a hexadecimal string, as per [EIP-155](/EIPS/eip-155). The `blockExplorerUrls`, `iconUrls`, and `rpcUrls` fields are arrays of strings, each of which MUST be a valid URL. The `nativeCurrency` field is an object with `name`, `symbol`, and `decimals` fields, where `decimals` is a non-negative integer, and is to be interpreted like in [EIP-20](/EIPS/eip-20). The `chainName` field is a string that is the human-readable name of the chain.
...
* Ensure that a submitted chain ID is valid.
  * It should be a `0x`-prefixed hexadecimal string per [EIP-695](/EIPS/eip-695), and parse to an integer number.
...
Erik Marks ([@rekmarks](https://github.com/rekmarks)), Pedro Gomes ([@pedrouid](https://github.com/pedrouid)), Pandapip1 ([@Pandapip1](https://github.com/Pandapip1)), "EIP-3085: wallet\_addEthereumChain RPC Method \[DRAFT]," *Ethereum Improvement Proposals*, no. 3085, November 2020. Available: https\://eips.ethereum.org/EIPS/eip-3085.
...
| command                                                                 | description               |
|-------------------------------------------------------------------------|---------------------------|
| curl.md https://eips.ethereum.org/EIPS/eip-3085 --objective <objective> | focus on a specific topic |
