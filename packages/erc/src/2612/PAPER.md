...
| Authors | Martin Lundfall ([@Mrchico](https://github.com/Mrchico)) |
| --- | --- |
| Created | 2020-04-13 |
| Requires | [EIP-20](/EIPS/eip-20), [EIP-712](/EIPS/eip-712) |
...
Arguably one of the main reasons for the success of [EIP-20](/EIPS/eip-20) tokens lies in the interplay between `approve` and `transferFrom`, which allows for tokens to not only be transferred between externally owned accounts (EOA), but to be used in other contracts under application specific conditions by abstracting away `msg.sender` as the defining mechanism for token access control.
...
For an improved user experience, the signed data is structured following [EIP-712](/EIPS/eip-712), which already has wide spread adoption in major RPC providers.
...
So far, many of these proposals have seen very little adoption, and the ones that have been adopted (such as [EIP-777](/EIPS/eip-777)), introduce a lot of additional functionality, causing unexpected behavior in mainstream contracts.

This ERC proposes an alternative solution which is designed to be as minimal as possible and to only address *one problem*: the lack of abstraction in the EIP-20 `approve` method.
...
Martin Lundfall ([@Mrchico](https://github.com/Mrchico)), "ERC-2612: Permit Extension for EIP-20 Signed Approvals," *Ethereum Improvement Proposals*, no. 2612, April 2020. Available: https\://eips.ethereum.org/EIPS/eip-2612.
...
| command                                                                 | description               |
|-------------------------------------------------------------------------|---------------------------|
| curl.md https://eips.ethereum.org/EIPS/eip-2612 --objective <objective> | focus on a specific topic |
