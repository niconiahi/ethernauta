...
const parametersSchema = union([
  ...
  object({ token: addressSchema, value: uint256Schema }),
])
type Parameters = InferOutput<typeof parametersSchema>

export function flashFee(_parameters: Parameters)
: Readable<Uint256> {
  return async (
    ...
    const values = Array.isArray(parameters)
      ? parameters
      : [parameters.token, parameters.value]
    const signature = build_signature("flashFee", [...PARAM_TYPES])
