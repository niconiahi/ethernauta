...
const parametersSchema = union([
  ...
  object({ receiver: addressSchema, token: addressSchema, amount: uint256Schema, data: bytesSchema }),
])
type Parameters = InferOutput<typeof parametersSchema>

export function flashLoan(_parameters: Parameters): Signable<Bytes> {
  return async ([signer, context]: ResolvedSigner): Promise<Bytes> => {
    ...
    const values = Array.isArray(parameters)
      ...
      : ([parameters.receiver, parameters.token, parameters.amount, parameters.data] as const)
