...
const PARAM_TYPES = ["uint256", "address"] as const
...
const parametersSchema = union([
  ...
  object({ assets: uint256Schema, receiver: addressSchema }),
])
type Parameters = InferOutput<typeof parametersSchema>

export function deposit(_parameters: Parameters)
: Signable<Bytes> {
  return async (
    ...
    const signature = build_signature("deposit", [...PARAM_TYPES])
