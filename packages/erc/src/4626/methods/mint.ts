...
export const MINT_SIGNATURE = {
  signature: "mint(uint256,address)",
  names: ["shares", "receiver"],
}

const parametersSchema = union([
  tuple([uint256Schema, addressSchema]),
  object({ shares: uint256Schema, receiver: addressSchema }),
])
type Parameters = InferOutput<typeof parametersSchema>

export function mint(_parameters: Parameters): Signable<Bytes> {
  return async ([signer, context]: ResolvedSigner): Promise<Bytes> => {
    ...
    const values = Array.isArray(parameters)
      ...
      : ([parameters.shares, parameters.receiver] as const)
