...
const PARAM_TYPES = ["address", "address", "uint256", "uint256", "bytes"] as const
...
const parametersSchema = union([
  ...
  object({ from: addressSchema, to: addressSchema, id: uint256Schema, value: uint256Schema, data: bytesSchema }),
])
type Parameters = InferOutput<typeof parametersSchema>

export function safeTransferFrom(_parameters: Parameters)
: Signable<Bytes> {
  return async (
    ...
    const values = Array.isArray(parameters)
      ? parameters
      : [parameters.from, parameters.to, parameters.id, parameters.value, parameters.data]
    const signature = build_signature("safeTransferFrom", [...PARAM_TYPES])
