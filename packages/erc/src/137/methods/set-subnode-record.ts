...
const parametersSchema = union([
  tuple([bytes32Schema, bytes32Schema, addressSchema, addressSchema, uint256Schema]),
  object({ node: bytes32Schema, label: bytes32Schema, owner: addressSchema, resolver: addressSchema, ttl: uint256Schema }),
])
...
export function setSubnodeRecord(_parameters: Parameters): Signable<Bytes> {
  return async ([signer, context]: ResolvedSigner): Promise<Bytes> => {
    ...
    const values = Array.isArray(parameters)
      ? parameters
      : [parameters.node, parameters.label, parameters.owner, parameters.resolver, parameters.ttl]
