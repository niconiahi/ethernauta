...
export const SAFE_TRANSFER_FROM_B88D4FDE_SIGNATURE = {
...
export function safeTransferFrom_b88d4fde(_parameters: Parameters): Signable<Bytes> {
  return async ([signer, context]: ResolvedSigner): Promise<Bytes> => {
    ...
    const values = Array.isArray(parameters)
      ...
      : ([parameters.from, parameters.to, parameters.tokenId, parameters.data] as const)
