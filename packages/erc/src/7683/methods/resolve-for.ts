...
const parametersSchema = union([
  tuple([object({ originSettler: addressSchema, user: addressSchema, nonce: uint256Schema, originChainId: uint256Schema, openDeadline: uint32Schema, fillDeadline: uint32Schema, orderDataType: bytes32Schema, orderData: bytesSchema }), bytesSchema]),
  object({ order: object({ originSettler: addressSchema, user: addressSchema, nonce: uint256Schema, originChainId: uint256Schema, openDeadline: uint32Schema, fillDeadline: uint32Schema, orderDataType: bytes32Schema, orderData: bytesSchema }), originFillerData: bytesSchema }),
])
...
export function resolveFor(_parameters: Parameters) {
  return (context: ContractContext): Callable<{ user: Address; originChainId: Uint256; openDeadline: Uint32; fillDeadline: Uint32; orderId: Bytes32; maxSpent: { token: Bytes32; amount: Uint256; recipient: Bytes32; chainId: Uint256 }[]; minReceived: { token: Bytes32; amount: Uint256; recipient: Bytes32; chainId: Uint256 }[]; fillInstructions: { destinationChainId: Uint64; destinationSettler: Bytes32; originData: Bytes }[] }> => {
    ...
    const values = Array.isArray(parameters)
      ...
      : ([parameters.order, parameters.originFillerData] as const)
