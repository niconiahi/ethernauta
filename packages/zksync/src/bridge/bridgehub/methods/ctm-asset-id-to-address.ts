...
export const CTM_ASSET_ID_TO_ADDRESS_SIGNATURE = {
  signature: "ctmAssetIdToAddress(bytes32)",
  names: ["ctmAssetId"],
}

const ParametersSchema = union([
  tuple([Bytes32Schema]),
  object({ ctmAssetId: Bytes32Schema }),
])
type Parameters = InferOutput<typeof ParametersSchema>

export function ctmAssetIdToAddress(
  ...
  return (context: ContractContext): Callable<Address> => {
    ...
    const values = Array.isArray(parameters)
      ...
      : ([parameters.ctmAssetId] as const)
