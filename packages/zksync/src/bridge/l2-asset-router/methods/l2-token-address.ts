...
export const L2_TOKEN_ADDRESS_SIGNATURE = {
  signature: "l2TokenAddress(address)",
  names: ["_l1Token"],
}

const ParametersSchema = union([
  tuple([AddressSchema]),
  object({ _l1Token: AddressSchema }),
])
type Parameters = InferOutput<typeof ParametersSchema>

export function l2TokenAddress(_parameters: Parameters) {
  return (context: ContractContext): Callable<Address> => {
    const parameters = parse(ParametersSchema, _parameters)
    const values = Array.isArray(parameters)
      ? ([parameters[0]] as const)
      : ([parameters._l1Token] as const)
