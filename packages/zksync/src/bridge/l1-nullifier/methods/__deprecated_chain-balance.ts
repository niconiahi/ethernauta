import {
  address,
  decode_function_result,
  encode_function_call,
  uint256,
} from "@ethernauta/abi"
import type { Bytes, Uint256 } from "@ethernauta/core"
import {
  AddressSchema,
  BytesSchema,
  Uint256Schema,
} from "@ethernauta/core"
import type {
  Callable,
  ContractContext,
} from "@ethernauta/transport"
import { bytes_to_hex } from "@ethernauta/utils"
import type { InferOutput } from "valibot"
import { object, parse, tuple, union } from "valibot"

const PARAM_CODECS = [uint256(), address()] as const
const OUTPUT_CODECS = [uint256()] as const

export const __DEPRECATED_CHAIN_BALANCE_SIGNATURE = {
  signature: "__DEPRECATED_chainBalance(uint256,address)",
  names: ["chainId", "l1Token"],
}

const ParametersSchema = union([
  tuple([Uint256Schema, AddressSchema]),
  object({
    chainId: Uint256Schema,
    l1Token: AddressSchema,
  }),
])
type Parameters = InferOutput<typeof ParametersSchema>

export function __DEPRECATED_chainBalance(
  _parameters: Parameters,
) {
  return (context: ContractContext): Callable<Uint256> => {
    const parameters = parse(ParametersSchema, _parameters)
    const values = Array.isArray(parameters)
      ? ([parameters[0], parameters[1]] as const)
      : ([parameters.chainId, parameters.l1Token] as const)
    const calldata = encode_function_call({
      name: "__DEPRECATED_chainBalance",
      args: PARAM_CODECS,
      values,
    })
    return {
      chain_id: context.chain_id,
      to: context.to,
      data: parse(BytesSchema, bytes_to_hex(calldata)),
      decode: (result: Bytes): Uint256 => {
        const [decoded] = decode_function_result(
          OUTPUT_CODECS,
          result,
        )
        return parse(Uint256Schema, decoded)
      },
    }
  }
}
