...
export const SET_UPGRADING_SIGNATURE = {
  signature: "setUpgrading(bool)",
  names: ["_upgrading"],
}

const ParametersSchema = union([
  tuple([boolean()]),
  object({ _upgrading: boolean() }),
])
...
export function setUpgrading(
  ...
  return async ([
    ...
    const values = Array.isArray(parameters)
      ...
      : ([parameters._upgrading] as const)
    ...
    return eth_signTransaction([
      {
        ...
        _ethernauta: {
          function: SET_UPGRADING_SIGNATURE,
        },
      },
    ])([signer, context])
  }
