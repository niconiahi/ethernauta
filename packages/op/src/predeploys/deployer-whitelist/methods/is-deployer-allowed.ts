...
export const IS_DEPLOYER_ALLOWED_SIGNATURE = {
  signature: "isDeployerAllowed(address)",
  names: ["_deployer"],
}

const ParametersSchema = union([
  tuple([AddressSchema]),
  object({ _deployer: AddressSchema }),
])
type Parameters = InferOutput<typeof ParametersSchema>

export function isDeployerAllowed(_parameters: Parameters) {
  return (context: ContractContext): Callable<boolean> => {
    const parameters = parse(ParametersSchema, _parameters)
    const values = Array.isArray(parameters)
      ? ([parameters[0]] as const)
      : ([parameters._deployer] as const)
