...
export const MINIMUM_GAS_LIMIT_SIGNATURE = {
  signature: "minimumGasLimit(uint64)",
  names: ["_byteCount"],
}

const ParametersSchema = union([
  tuple([Uint64Schema]),
  object({ _byteCount: Uint64Schema }),
])
type Parameters = InferOutput<typeof ParametersSchema>

export function minimumGasLimit(_parameters: Parameters) {
  return (context: ContractContext): Callable<Uint64> => {
    const parameters = parse(ParametersSchema, _parameters)
    const values = Array.isArray(parameters)
      ? ([parameters[0]] as const)
      : ([parameters._byteCount] as const)
