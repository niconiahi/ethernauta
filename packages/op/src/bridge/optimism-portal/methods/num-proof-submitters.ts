...
export const NUM_PROOF_SUBMITTERS_SIGNATURE = {
  signature: "numProofSubmitters(bytes32)",
  names: ["_withdrawalHash"],
}

const ParametersSchema = union([
  tuple([Bytes32Schema]),
  object({ _withdrawalHash: Bytes32Schema }),
])
type Parameters = InferOutput<typeof ParametersSchema>

export function numProofSubmitters(
  ...
  return (context: ContractContext): Callable<Uint256> => {
    ...
    const values = Array.isArray(parameters)
      ...
      : ([parameters._withdrawalHash] as const)
