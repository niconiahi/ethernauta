...
const SELECTOR = function_selector("bridgeERC20To", [
  address_codec(),
  address_codec(),
  address_codec(),
  uint256_codec(),
  ...
  return {
    ...
    destination: {
      ...
      reader: async (_call: Call): Promise<Response> => {
        throw new Error(
          "destination reader should not be invoked from send_erc20",
        )
      },
    },
  }
...
describe("send_erc20", () => {
  it("signs eth_signTransaction with bridgeERC20To calldata and zero value, then broadcasts via eth_sendRawTransaction and returns the L1 tx hash", async () => {
