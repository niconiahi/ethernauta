...
const L2_TO_L1_MESSAGE_PASSER = parse(
  AddressSchema,
...
const PAYLOAD = parse(
  BytesSchema,
  "0xa9059cbb0000000000000000000000001111111111111111111111111111111111111111000000000000000000000000000000000000000000000000016345785d8a0000",
)
...
const SELECTOR = function_selector("initiateWithdrawal", [
  address_codec(),
  uint256_codec(),
  bytes_codec(),
])
  ...
  return {
    ...
    destination: {
      ...
      reader: async (_call: Call): Promise<Response> => {
        throw new Error(
          "destination reader should not be invoked from start_withdraw_message",
        )
      },
    },
  }
...
describe("start_withdraw_message", () => {
  it("signs initiateWithdrawal with msg.value mirroring the value param, broadcasts on L2, returns the tx hash", async () => {
    ...
    const value = parse(Uint256Schema, "0x16345785d8a0000")
    const gas_limit = parse(Uint256Schema, "0x30d40")
    ...
    expect(signed.tx.to).toBe(L2_TO_L1_MESSAGE_PASSER)
    ...
    const expected_calldata = bytes_to_hex(
      encode_function_call({
        ...
        values: [RECIPIENT, gas_limit, PAYLOAD],
      }),
    )
  ...
  it("throws when origin.signer is undefined", async () => {
    ...
    await expect(
      start_withdraw_message({
        to: RECIPIENT,
        value: parse(Uint256Schema, "0x16345785d8a0000"),
        gas_limit: parse(Uint256Schema, "0x30d40"),
        data: PAYLOAD,
      })(resolved),
