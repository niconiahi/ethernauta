  ...
  return {
    ...
    destination: {
      ...
      reader: async (_call: Call): Promise<Response> => {
        throw new Error(
          "destination reader should not be invoked from start_withdraw_erc20",
        )
      },
    },
  }
...
describe("start_withdraw_erc20", () => {
  it("signs withdrawTo with msg.value = 0, broadcasts on L2, returns the tx hash", async () => {
    ...
    expect(signed.tx.value).toBe(ZERO_VALUE)
