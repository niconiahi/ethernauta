...
import { type Description, DescriptionSchema } from "./abi/description"
...
function parse_signature(_sig: string): Description {
  const sig = _sig.trim()
  if (sig.startsWith("function ")) return parse_function(sig)
  if (sig.startsWith("event ")) return parse_event(sig)
  if (sig.startsWith("error ")) return parse_error(sig)
  if (sig.startsWith("constructor")) return parse_constructor(sig)
  throw new Error(`parse_abi: unrecognised signature "${_sig}"`)
}
...
function parse_function(_sig: string): Description {
  ...
  const { args, rest } = split_arglist(after_kw.slice(name_end))
  ...
  for (const m of ["pure", "view", "payable", "nonpayable"] as const) {
  ...
  if (cursor.startsWith("returns")) {
    ...
    const { args: out_args, rest: tail } = split_arglist(cursor)
    outputs = out_args.map((arg) => parse_arg(arg, false))
    cursor = tail.trim()
  }
  if (cursor.length > 0) {
    throw new Error(`parse_abi: trailing tokens "${cursor}" in "${_sig}"`)
  }
  return parse(DescriptionSchema, {
    type: "function",
    name,
    inputs: inputs.map(({ name: n, type }) => ({ name: n, type })),
    outputs: outputs.map(({ name: n, type }) => ({ name: n, type })),
...
function parse_event(_sig: string): Description {
  ...
  if (!anonymous && rest.trim().length > 0) {
    throw new Error(`parse_abi: trailing tokens in "${_sig}"`)
  }
  return parse(DescriptionSchema, {
    type: "event",
    name,
...
type ParsedArg = { name: string; type: string; indexed?: boolean }

function parse_arg(_arg: string, allow_indexed: boolean): ParsedArg {
  ...
  if (i < tokens.length) {
    throw new Error(`parse_abi: unexpected tokens in "${_arg}"`)
  }
  return allow_indexed ? { name, type, indexed } : { name, type }
}

// Pull `(...)` from the start of `s`, returning the comma-split arg
// strings and the remainder of `s` after the closing paren.
function split_arglist(_s: string): { args: string[]; rest: string } {
  const s = _s.trimStart()
  if (!s.startsWith("(")) {
    throw new Error(`parse_abi: expected "(", got "${s.slice(0, 8)}…"`)
  }
  const close = find_matching_paren(s, 0)
  const inner = s.slice(1, close).trim()
  const rest = s.slice(close + 1)
  if (inner.length === 0) return { args: [], rest }
  return { args: inner.split(",").map((p) => p.trim()), rest }
}

function find_matching_paren(_s: string, _start: number): number {
  let depth = 0
