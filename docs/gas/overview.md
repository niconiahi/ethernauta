...
> ⓘ **OP Stack and Arbitrum moved.** The OP-specific gas helper relocated into [`@ethernauta/op`](/op/overview) as `estimate_op_fees` (see [`/op/gas`](/op/gas)). The Arbitrum-specific gas helper relocated into [`@ethernauta/arbitrum`](/arbitrum/overview) as `estimate_arbitrum_fees` (see [`/arbitrum/gas`](/arbitrum/gas)). zkSync still lives here until its family package lands.
...
For the 1559 baseline (`estimate_priority_fee`, `estimate_1559_fees`, `buffer_gas_limit`) see [`/eth/overview`](/eth/overview) — those moved into `@ethernauta/eth` alongside the rest of the substrate fee surface. For OP Stack (`estimate_op_fees`, `estimate_l1_fee`) see [`/op/gas`](/op/gas). For Arbitrum (`estimate_arbitrum_fees`) see [`/arbitrum/gas`](/arbitrum/gas).

## L2 families

zkSync is the only family still served from this package. Arbitrum moved to [`@ethernauta/arbitrum`](/arbitrum/gas); OP Stack moved to [`@ethernauta/op`](/op/gas).
...
- [`/arbitrum/gas`](/arbitrum/gas) — Arbitrum fee estimation (`estimate_arbitrum_fees`). Moved out of `@ethernauta/gas`.
