...
Most of the time you don't construct the wire bytes yourself — [`@ethernauta/gas`](/gas/overview)'s `estimate_1559_fees` composes the fee triple, and the wallet's `eth_sendTransaction` handler does the encoding. You reach for these primitives when you need:
...
- [@ethernauta/gas](/gas/overview) — the fee-estimation layer built on these primitives.
