...
```ts
import type { DiscoverOptions } from "@ethernauta/eip/6963";

const _options: DiscoverOptions = { target: window, ms: 100 };
// shape: { target?: EventTarget; ms?: number }
```
...
if (persisted) {
  ...
  const adapter = create_provider(persisted.provider);
  void adapter;
} else {
